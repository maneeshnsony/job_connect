  ...
  return (
    <div className="w-full max-w-sm rounded-xl shadow-lg border p-8" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
      ...
      <p className="mt-6 text-center text-sm" style={{ color: 'var(--text-muted)' }}>
        ...
        <button onClick={() => router.push('/register')} className="text-blue-600 hover:underline font-medium">Sign up</button>
      </p>
    </div>
  );
}
...
export default function LoginPage() {
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg-secondary)' }}>
      <Suspense fallback={<div className="animate-spin rounded-full h-8 w-8 border-2 border-blue-600 border-t-transparent"></div>}>
        <LoginForm />
      </Suspense>
    </div>
  );
